...
//  Goopal::getInstance()->appendCurrentDialogVector(this);
    ...
    ui->checkBtn->setStyleSheet("QToolButton{background-color:#ffffff;color:#282828;border:1px solid #7dd9df;border-radius:3px;}"
                                "QToolButton:hover{color:#7dd9df;}");
...
void NameDialog::setName(const QString& name)
{
...
void NameDialog::on_checkBtn_clicked()
{
    ...
    if (DataMgr::getInstance()->walletCheckNameExists(addrName)) {
		ui->okBtn->setEnabled(false);
        ui->addressNameTipLabel1->setPixmap(QPixmap(DataMgr::getDataMgr()->getWorkPath() + "pic2/wrong.png"));
