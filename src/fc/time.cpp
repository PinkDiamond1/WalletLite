...
namespace fc {
 ...
 //   return boost::posix_time::to_iso_string( ptime );
      std::time_t now_c(sec_since_epoch());
      std::stringstream s;
      s<<std::put_time(std::gmtime(&now_c), "%Y-%m-%dT%H:%M:%S");
      return s.str();
  }
  ...
  { try {
   ...
   //       pt = boost::date_time::parse_delimited_time<boost::posix_time::ptime>( s, 'T' );
    ...
    //      pt = boost::posix_time::from_iso_string( s );
      ...
      std::chrono::system_clock::time_point tm_pt = std::chrono::system_clock::from_time_t(std::mktime(&t));
     return time_point( microseconds( bch::duration_cast<bch::microseconds>(tm_pt.time_since_epoch()).count() ) );
